...
namespace Trust.Dungeons
{
    ...
    public class HallOfTheNoviceArena : AbstractDungeon
    {
        ...
        /// </summary>
        public new const ZoneId ZoneId = Data.ZoneId.HallOfTheNoviceArena;

        private const int QuickFistedTrainingPartner = 4786;
        private const int TamedJackal = 4787;

        private const float AttackRange = 20.0f;
        private static readonly Dictionary<ClassJobType, uint> JobSpecificAction = new Dictionary<ClassJobType, uint>
        {
        ...
        public override async Task<bool> RunAsync()
        {
            ...
            foreach (BattleCharacter target in targets)
            {
                ...
                {
                    Logging.WriteDiagnostic($"Drawing aggro of {target.Name} ({target.NpcId})");
                    ...
                    if (JobSpecificAction.TryGetValue(Core.Player.CurrentJob, out uint actionId))
                    {
                        ...
                        Logging.WriteDiagnostic($"Casting {action.Name} ({action.Id}) on {target}");
