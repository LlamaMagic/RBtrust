...
namespace Trust.Dungeons
{
    ...
    public class KtisisHyperboreia : AbstractDungeon
    {
        ...
        private readonly Stopwatch frostBiteAndSeekSw = new Stopwatch();
        ...
        private readonly Stopwatch hermesTrueAeroIITimer = new Stopwatch();
        ...
        public override async Task<bool> RunAsync()
        {
            ...
            if (WorldManager.SubZoneId == (uint)SubZoneId.FrozenSphere)
            {
                if (frostBiteAndSeek.IsCasting())
                {
                    ...
                    Logging.Write(Colors.Red, "stopwatchPassed: " + frostBiteAndSeekSw.ElapsedMilliseconds);
            ...
            if (WorldManager.SubZoneId == (uint)SubZoneId.CelestialSphere)
            {
                ...
                {
                    Logging.Write(Colors.Red, $"True Aero II timer: {hermesTrueAeroIITimer.ElapsedMilliseconds:N0}ms");
                    SidestepPlugin.Enabled = false;
                    AvoidanceManager.RemoveAllAvoids(i => i.CanRun);
                    moveToSafety = true;
                    while (moveToSafety)
